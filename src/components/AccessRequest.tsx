...
import { accessLadder, accessLevels } from "../data/accessLevels";
...
export default function AccessRequest() {
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    const levelLabel = accessLevels[form.level as keyof typeof accessLevels]
      ? accessLevels[form.level as keyof typeof accessLevels].label
      : form.level;
    const body = [
      ...
      <div className="container-px">
        <div className="grid gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
          <Reveal>
            <div>
              <SectionTitle
                eyebrow="Institutional Access Request"
            ...
            {submitted ? (
              <div className="panel flex h-full flex-col items-center justify-center p-10 text-center">
                ...
                <p className="mt-2 max-w-sm text-sm leading-relaxed text-ivory/65">
                  Your email client has been opened with a pre-filled request to{" "}
                  <span className="text-sand">{contact.email}</span>. If it did
                  not open, please send your details to that address. Access is
                  reviewed manually for institutional purposes.
                </p>
                <button
                  onClick={() => {
                    setForm(initial);
              ...
              <form onSubmit={onSubmit} className="panel p-7">
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    ...
                    placeholder="Briefly describe the institutional purpose of your request."
                  />
                </div>
                ...
                <button type="submit" className="btn-primary mt-6 w-full">
                  <Mail className="h-4 w-4" /> Submit Access Request
                </button>
                <p className="mt-3 text-center text-xs text-ivory/40">
                  No data is stored. This opens a pre-filled email to{" "}
                  {contact.email}.
                </p>
              </form>
            )}
